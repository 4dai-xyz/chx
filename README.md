...
```text
ORB-SLAM3 官方源码、Vocabulary 文件和编译产物不应提交到本仓库。
...
```text
不要提交 src/KV-tracker/thirdparty/。
不要提交 SAM2 checkpoint、Pi3 示例视频或其他第三方模型文件。
```
...
5. 渲染类似论文 Reinforced Cross-Modal Matching and Self-Supervised Imitation Learning
for Vision-Language Navigation Figure 1 的俯视展示：
   - 全局场景 / 地图；
   - 起点和终点；
   - 相机轨迹；
   - 动态行人 overlay；
...
3. 基于地图物理约束的动态行人过滤。
4. 类似论文 Reinforced Cross-Modal Matching and Self-Supervised Imitation Learning
for Vision-Language Navigation Figure 1 的俯视路线图展示。
